...
def main(
    ...
    plt.plot((np.array(history).reshape(100, -1) == 1).mean(axis=0), alpha=0.4)
    plt.plot((np.array(history).reshape(100, -1) == -1).mean(axis=0), alpha=0.4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", help="number of rows", type=int, default=3)
    parser.add_argument("-c", help="number of columns", type=int, default=3)
    parser.add_argument("-e", help="epsilon", type=float, default=0.2)
    parser.add_argument("-a", help="alpha", type=float, default=0.4)
    parser.add_argument("-d", help="discount rate", type=float, default=0.9)
    parser.add_argument("--smi", help="min of score", type=int, default=1)
    parser.add_argument("--sma", help="max of score", type=int, default=9)
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        "--mp", help="path to the output pickle", type=str, default="model.pickle"
    )
    parser.add_argument(
        "--lp", help="path to the output picture", type=str, default="win-rate.png"
    )
    args = parser.parse_args()

    qlearnagent = QLearningAgent(
        "q-learning", epsilon=args.e, alpha=args.a, discount=args.d
    )
    if args.o == "r":
